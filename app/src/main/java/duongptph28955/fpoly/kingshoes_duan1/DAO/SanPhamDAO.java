...
public class SanPhamDAO {
    ...
    public ArrayList<SanPham> getDSSanPham(){
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT sp.maSanPham, lg.maLoai, lg.tenLoai, sp.tenSanPham, sp.giaNhap, sp.soLuong, sp.ngayNhap, sp.tenMau, sp.tenSize, sp.hinhAnh FROM SANPHAM sp, LOAIGIAY lg WHERE sp.maLoai = lg.maLoai", null);
        if (cursor.getCount() != 0){
            cursor.moveToFirst();
            do {
                list.add(new SanPham(cursor.getInt(0), cursor.getInt(1), cursor.getString(2), cursor.getString(3), cursor.getInt(4), cursor.getInt(5), cursor.getString(6),cursor.getString(7), cursor.getString(8), cursor.getBlob(9)));
            }while (cursor.moveToNext());
        }
        return list;
    }

    public boolean themGiayMoi(String tensp, int maloai, int gianhap, int soluong, String ngaynhap,String tenMau, String tenSize, byte[] hinh){
        ...
        contentValues.put("tenMau", tenMau);
